...
<body class="container text-center bg-dark">
    ...
    @for ($i = 0; $i < count($data); $i++)
        <card nome="{{ $data[$i]->name }}" especie="{{ $data[$i]->name }}" imagem="{{ $data[$i]->image }}"
            id="{{ $data[$i]->id }}" genero="{{ $data[$i]->gender }}" nome_original="{{ $data[$i]->origin->name }}"
            localizacao="{{ $data[$i]->location->name }}" status="{{ $data[$i]->status }}"
            episode="{{ $primeirosEpisodios[$i] }}"></card>
    @endfor
    </div>
    <Async></Async>
    <form method="GET" action={{ route('home') }} class="async-form">
        ...
        <div class="btn-group" role="group" aria-label="Basic example">
            <button type="submit" class="btn btn-primary" id="prevBtn" name="id" value={{ $pagina - 1 }}><svg
            ...
            <button type="submit" class="btn btn-primary" id="nextBtn" name="id"
                value={{ $pagina + 1 }}>Próximo <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
